...
public class SpuInfoServiceImpl extends ServiceImpl<SpuInfoDao, SpuInfoEntity> implements SpuInfoService {
        ...
        if(skus!=null && skus.size()>0){
            skus.forEach(item->{
                ...
                List<SkuImagesEntity> imagesEntities = item.getImages().stream().map(img -> {
                    ...
                    return !StringUtils.isEmpty(enti.getImgUrl());
                }).collect(Collectors.toList());
    ...
    public void up(Long spuId) {
        ...
        // TODO: 2022/1/21 4、查询当前sku的所有可以被用来检索的规格属性
        ...
        List<SkuEsModel> upProducts = skus.stream().map(sku -> {
            ...
            // TODO: 2022/1/21 2热度评分 0
            esModel.setHotScore(0L);

            // TODO: 2022/1/21 3查询品牌和分类的名字信息
        ...
        try {
            // TODO: 2022/1/21 5、将数据发送给es进行保存；gulimall-search
            R r = searchFeignService.productStatusUp(upProducts);
            if(r.getCode()==0){
                //远程调用成功
                // TODO: 2022/1/21 6\修改当前spu状态
